...
// 🔁 REMPLACE TOUTE TA ROUTE /timecard-pdf PAR CE BLOC
router.post('/timecard-pdf', async (req, res) => {
  ...
  try {
    ...
    // Si le front a déjà envoyé rows enrichies, on s'en sert, sinon on retombe sur timeCards
    const source = rows.length ? rows : timeCards;

    // Résolution robuste helper/replacement pour éviter les inversions et doublons
    const resolveReplacement = (primaryEmployeeId) => {
      const d = disponibilities?.[primaryEmployeeId];
      const partnerType = (d?.partnerType || '');
      const partnerEmployeeId = (d?.partnerEmployeeId || ''); // côté back: id de la personne REMPLACÉE

      let effectiveDriverId = primaryEmployeeId; // par défaut: c'est le planifié qui a travaillé
      let replacedEmployeeId = '';

      if (partnerType === 'replacement') {
        // partnerEmployeeId est la personne REMPLACÉE
        if (partnerEmployeeId && String(partnerEmployeeId) !== String(primaryEmployeeId)) {
          ...
          // cas pathologique: on tente de trouver un enregistrement "miroir" où l'employé effectif apparaît
    ...
    const tableRows = source.map((r) => {
      ...
      // Si le front a déjà calculé, on reprend; sinon on résout côté back
      ...
      // Empêcher doublon même nom (si bug amont)
      ...
      return {
        ...
        additionalInfo,           // affiché en dernière colonne
    ...
    doc.autoTable({
      ...
      body: tableRows.map(tr => ([
        ...
        tr.tel,
        tr.powerbank,
        tr.refueled ? 'Yes' : 'No',   // ➕ AJOUT (ou 'Yes'/'No')
        tr.comment,
        tr.additionalInfo || '',
      ])),
      styles: {
...
/**
 * Récupère tous les jours de training créés à une date spécifique
 * @param {string} dateString - Format "YYYY-MM-DD"
 * @returns {Promise<Array>} Liste des day[] de training pour ce jour-là
 */
  ...
  allTrainings.forEach(training => {
    ...
    days.forEach(day => {
      try {
        ...
        if (dayDate !== targetDate) return; // ⛔ Ignorer les autres jours
        ...
        console.error(`⚠️ Erreur parsing ObjectId: ${err.message}`);
      }
    });
  });
  return results;
